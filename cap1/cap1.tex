...
Il TPS RayStation in particolare implementa un algoritmo di tipo convolution/superposition conosciuta come \textit{collapsed cone convolution} sviluppato, a partire dalla metà dagli anni '80, indipendentemente da Mackie e Ahnesj{\"{o} \cite{Ahnesjo1989, Boyer1998, Mackie1985, Ahnesjo1987}.
...
Applicando le funzioni di scatter (kernel) ai punti di deposizione del TERMA, in linea di principio è possibile arrivare alla dose assorbita in maniera diretta risolvendo per via numerica l'integrale \eqref{eq:superp}. Tuttavia, questa operazione su una griglia di $N^3$ voxel risulta essere un problema  di ordine $N^7$ \cite{Ahnesjo1999}, difficilmente gestibile anche con le moderne potenze di calcolo\footnote{Se immaginiamo di discretizzare un paziente di dimensioni $20x20x20$ cm$^3$ con voxel di lato $0.3$ cm otteniamo una griglia di circa $70x70x70$ voxel. Il numero di operazioni da effettuare risolvendo numericamente l'integrale \eqref{eq:superp} sarebbe $70^7\approx 10^{13}$. Disponendo di un computer capace di effettuare operazioni con frequenza dell'ordine del GHz$\equiv 10^9$ il tempo di risolvere $10^{13}$ operazioni sarebbe $t=10^{13}/10^9=10^5\,sec\approx 2\, ore!$.}. 

Un possibile approccio alternativo alla risoluzione della \eqref{eq:superp} si basa sulla considerazione per cui, nel caso di un mezzo omogeneo, la funzione di scatter della \eqref{eq:superp} risulta essere spazialmente invariante. Questo permette l'applicazione della teoria degli spazi di Fourier per cui l'integrale di convoluzione si trasforma in un semplice prodotto delle trasformate delle funzioni coinvolte. Tutto ciò è valido per uno spazio continuo. L'implementazione su computer di questo metodo (come di un qualsiasi altro) presuppone però una discretizzazione dello spazio di integrazione e delle funzioni da integrare. Vedremo nella prossima sezione come questo sia un passaggio delicato da trattare e sul quale viene applicata l'approssimazione fondamentale dell'algoritmo dosimetrico implementato nel TPS RayStation.


\subsection{L'approssimazione \textit{collapsed-cone}}
Lo stesso Mackie assieme a Reckwerdt \cite{Reckwerdt1992} propose una discretizzazione del problema \eqref{eq:superp} che consisteva nel calcolare l'integrale \eqref{eq:superp} solo lungo determinate direzioni. Indipendentemente, Ahnesj\"{o} nel 1989 seguendo lo stesso approccio, pubblicò un articolo \cite{Ahnesjo1989} in cui propose il metodo denominato \textit{collapsed-cone} e lo confrontò con il metodo Monte Carlo ottenendo risultati eccellenti per l'epoca. Questi risultati rappresentano ancora oggi il benchmark per il calcolo della dose con TPS utilizzando metodi non statistici.
\begin{figure}[!t]
\centering
...
\begin{figure}
\centering
\includegraphics[width=.4\textwidth]{./cap1/Kernel_RayTr.eps}
\includegraphics[width=.45\textwidth]{./cap1/Kernel_CCC.eps}
...
\end{figure}
Tuttavia Ahnesj\"{o} notò che questo metodo semplicistico di discretizzazione può portare ad un errore di sampling ampio a causa della rapidità di rilascio di energia nei primi cm di interazione \cite{Ahnesjo1989} (i.e. una larga parte di deposizione di energia non viene conteggiata a meno di usare un campionamento angolare fittissimo complicato da gestire a livello computazionale (vedi Fig.\ref{fig:raytrace_vs_cc})).\\
...
Mackie \cite{Mackie1985} dimostrò come sia possibile evitare di ricalcolare i kernel per ogni situazione sostituendo tutte le lunghezze fisiche ($l$) con le lunghezze radiologiche ($\rho\,l$) (dove $\rho$ è la densità del mezzo). La base di questa metodologia era in realtà nota già prima dell'avvento degli algoritmi di convolution come Teorema di O'Connor \cite{OConnor1957}.\\
\begin{figure}[!t]
\centering
...
\label{fig:kern_dens}
\end{figure}
Nella Fig.\ref{fig:kern_dens} si può notare l'errore che si commette adottando il metodo suggerito in una configurazione di disomogeneità aria/acqua che può simulare una situazione tessuto/polmone (Woo e Cunningham \cite{Woo1990}). Nonostante l'accordo non sia perfetto, vari autori come Arnfield et al. \cite{Arnfield2000} e lo stesso Ahnesj\"{o} \cite{Ahnesjo1989} hanno dimostrato che su un insieme numeroso di interazioni interviene un effetto di \textit{averaging} per cui discrepanze significative rimangono solo in condizioni di estrema disomogeneità, ossia vicino alle interfacce in cui la densità cambia repentinamente (vedi Fig.\ref{fig:kern_dens}).
...
Considerando un mezzo disomogeneo, la funzione di scatter perde la sua invarianza spaziale per cui l'equazione \eqref{eq:superp} cessa di essere un'integrale di convoluzione. Vedremo nel capitolo successivo come questa condizione matematica rispecchi il fenomeno fisico cosiddetto della perdita di equilibrio di particelle cariche\footnote{L'equilibrio di particelle cariche (CPE) è una condizione per cui, considerato una superficie chiusa, il numero di particelle ionizzanti che entrano nella superficie è uguale al numero che ne esce.}.

\`E tuttavia possibile recuperare l'invarianza dei kernel di deposizione grazie all'applicazione del teorema di O'Connor, sostituendo le lunghezze fisiche con le lunghezze radiologiche \cite{Khan2010}. Questa operazione è nota come principio di \textit{superposition} e dà luogo all'equazione:
\begin{equation}
