...
Stabilendo un angolo solido $\Omega_i$, l'operazione di \virg{collapsing} è un'integrazione in coordinate sferiche del kernel \eqref{eq:kern_fit}:
\begin{equation}
...
Il metodo collapsed-cone è stato dimostrato \cite{Ahnesjo1999} comportare un numero di operazioni dell'ordine di $MN^3$ dove $M$ è il numero di settori in cui viene diviso l'angolo solido ($N$ è il numero di voxel della griglia di dose/TERMA). Questo è da confrontare con l'ordine $N^7$ in caso di applicazione triviale del metodo di sovrapposizione.
\begin{figure}
\centering
...
\caption{Sinistra:Contronto tra un kernel calcolato direttamente con un metodo Monte Carlo (linea solida) e un kernel calcolato in condizioni omogenee e riscalato secondo la lunghezza radiologica (linea tratteggiata); le isolinee corrispondono ad una dose relativa di 1000, 500, 100, 50, 10, 5, 1; il punto A posizionato appena sotto l'interfaccia aria/acqua presenta la massima discrepanza (50\%). Destra: effetto globale su un insieme di interazioni; un \textit{averaging} delle incertezze sui singoli kernel si risolve in un accordo accettabile \cite{Woo1990,Arnfield2000,Ahnesjo1989}.}
\label{fig:kern_dens}
\end{figure}
...
CONTINUA VEDENDO SE METTERE L'EQAUZIONE CON RHO (dal KHAN) e poi prosegui con un paragrafo riepilogativo dove parli della superposition finale.

Poi metti una nuova sezione sulle approssimazioni di speeding up: kernel tilt e interpolazione adattiva.


Poi metti gli elettroni per chiudere
