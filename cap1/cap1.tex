...
\begin{itemize}
\item Si parte dalla mappa di fluenza calcolata sul piano passante per l'isocentro e perpendicolare alla direzione di propagazione del fascio senza considerare la presenza del paziente (Sez.sec:fluence).
...
\end{equation}
L'integrale del coefficiente di assorbimento nella precedente equazione viene discretizzato nel TPS su una griglia di voxel cubici detta \textit{dose grid}. Questi voxel campionano le densità fornite dalla CT da cui viene ricavato il coefficiente di assorbimento locale $\mu / \rho(\vec{r'},E)$. Moltiplicando questa quantità con la \eqref{eq:fluence} si ottiene la distribuzione di TERMA (Eq.\eqref{eq:termaE}).
...
Un possibile approccio alternativo alla risoluzione della \eqref{eq:superp} si basa sulla considerazione per cui, nel caso di un mezzo omogeneo, la funzione di scatter della \eqref{eq:superp} risulta essere spazialmente invariante. Questo permette l'applicazione della teoria degli spazi di Fourier per cui l'integrale di convoluzione si trasforma in un semplice prodotto delle trasformate delle funzioni coinvolte. Tutto ciò è valido per uno spazio continuo. L'implementazione su computer di questo metodo (come di un qualsiasi altro) presuppone una discretizzazione dello spazio e delle funzioni. Vedremo nella prossima sezione come questo sia un passaggio delicato da trattare e sul quale viene applicata l'approssimazione fondamentale dell'algoritmo dosimetrico denominato \textit{collapsed-cone}.
...
Per capire il meccanismo alla base di questa approssimazione partiamo con l'osservare in Fig.\ref{fig:kern_ray} la distribuzione spaziale tipica di alcuni kernel di deposizione. La discretizzazione di questi kernel può essere effettuata in maniera diretta lungo delle semplici linee di integrazione seguendo un processo noto come \textit{ray-tracing} (vedi Fig.\ref{fig:raytrace_vs_cc}a).
\begin{figure}
\centering
INSERISCI QUI LA FIGURA DEL RATRACE VS CCC
%\includegraphics[width=\textwidth]{}
\caption{}
\label{fig:raytrace_vs_cc}
\end{figure}

Tuttavia Ahnesj\"{o} notò che questo metodo triviale di discretizzazione può portare ad un errore di sampling ampio a causa della rapidità di rilascio di energia nei primi cm di interazione \cite{Ahnesjo1989} (i.e. una larga parte di deposizione di energia non viene conteggiata a meno di usare un campionamento angolare fittissimo).\\
