...
\subsection{La sovrapposizione e la convoluzione in termini matematici}
Le operazioni di sovrapposizione e di convoluzione sono concetti matematici largamente utilizzati in fisica. Ragionando unicamente in termini matematici, l'operazione di sovrapposizione consiste nella somma di una serie di funzioni, ognuna con un proprio peso.\\
Nel caso continuo, l'operazione di sovrapposizione può essere espressa con un integrale di volume tra una funzione primaria $p$ ed una funzione kernel $s$:
\begin{equation}
...
Quest'ultima equazione risulta di più facile trattazione numerica grazie alla teoria degli spazi di Fourier.
...
Questa equazione è valida nel caso di un fascio di fotoni monoenergetico. La generalizzazione al caso polienergetico è semplice introducendo il TERMA differenziale in energia $T(P',E)$ e la funzione di scatter polienergetica $s(P'\rightarrow P,E)$ ed integrando su tutte le energie coinvolte:
\begin{equation}
D(P) = \iint_{E,V} T(P',E)\,s(P'\rightarrow P,E)\, \de V \de E
\label{eq:superp}
\end{equation}
L'equazione \eqref{eq:superp} matematicamente è della forma di un integrale di sovrapposizione di due funzioni espresso nell'Eq.\eqref{eq:sovrapp_math}.
...
\end{equation}
che rappresenta il cosiddetto \textit{principio di convolution/superposition} applicato al calcolo della dose in un volume investito da un fascio di fotoni polienergetico.
...
L'approssimazione di collapsed cone consiste in una discretizzazione del kernel che è necessaria per poter risolvere l'integrale \eqref{eq:superp} per via computazionale. Ahnesj\"{o} notò che un metodo triviale di discretizzazione con dei semplici raggi, può portare ad un errore di sampling ampio a causa della rapidità di rilascio di energia nei primi cm di interazione \cite{Ahnesjo1989} (Fig.\ref{fig:kern_ray}) (i.e. una larga parte di deposizione di energia non viene conteggiata a meno di usare un campionamento angolare fittissimo).\\
...
\end{equation}
Questa equazione rappresenta il \textit{principio di convolution} per il calcolo della dose. Considerando un mezzo infinitamente esteso, la funzione $s$ è spazialmente invariante per cui l'integrale è facilmente risolvibile grazie alla teoria degli spazi di Fourier.

Aggiungendo la generalizzazione al caso disomogeneo, la \eqref{eq:superp_homo} può assume la forma \cite{Khan2010}:
\begin{equation}
