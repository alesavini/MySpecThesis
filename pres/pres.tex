...
\centering
\vspace{.2cm}
{\color{Dgreen} \bf Criterio di accettazione: $\gamma < 1$}
\end{frame}
...
\end{table}
\begin{itemize}
\item<2-> A seguito dei risultati positivi ottenuti il sistema di elaborazione di piani di trattamento è stato posto in uso clinico presso la ASL di Teramo.
\end{itemize}
\end{frame}
...
\begin{frame}{bla bla}
Parlare del fatto che ora sappiamo che in quel pixel c'è la dose giusta entro una precisione tollerabile clinicamente\\

allora il commissioning è stato esteso anche alla radioterapia adattiva, definirla, dire che si è preso parte a due studi multi centrici italiani in corso di svolgimento per mettere in atto queste tecniche di innovative in cui il piano di trattamento non è sempre lo stesso ma si adatta ai cambiamenti del paziente...
\end{frame}
