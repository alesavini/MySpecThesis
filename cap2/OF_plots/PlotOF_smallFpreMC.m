...
% invece va bene in png per JACMP 11.25cm = 4.2 inches
    ...
    hold on
    plot(CC01(:,1),CC01(:,2),'*:m');
    plot(PFD(:,1),PFD(:,2),'p-r');
    plot(SFD(2:end,1),SFD(2:end,2),'s--b');
    plot(W1(:,1),W1(:,2),'+:c');
...
%         'LineStyle', '--', 'Color', 'black')
    ...
    title('Monte Carlo correction factors not applied','FontSize',10)
...
%     title(s,'FontSize',10,'FontWeight','n');
    ...
    print(fig,'OFSmallpreMC','-dpng','-r150')
    saveas(fig,'OFSmallpreMC','epsc')