...
% invece va bene in png per JACMP 11.25cm = 4.2 inches
    ...
    plot(prof_sum_meas(:,1)./10,prof_sum_meas(:,2),'LineWidth',1,'MarkerSize',20);
    plot(prof_sum_calc(:,1)./10,prof_sum_calc(:,2).*1.01,'LineWidth',1,'MarkerSize',20);
...
%     hlCoarse = line(AnglesAbscissa,coarse,'LineWidth',1.5,'Color','g');    
    ...
    lh1 = legend('Measured', 'Calculated', 'Location','NorthEast');
    legend('boxoff')
...
%     print(fig,'abutted_m','-dpng','-r150')
    saveas(fig,'PlotMLC_Tip_modeling_offaxisSUM','epsc')