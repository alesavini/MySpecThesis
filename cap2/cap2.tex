...
\item[Distanza di accordo:] fissato un punto nello spazio individuato dal vettore $\vec{r}_{meas}$ corrispondente ad una certa dose misurata, la distanza di accordo rappresenta la minima distanza tra $\vec{r}_{meas}$ e tutti i punti aventi la medesima dose calcolata individuati dai vettori $\vec{r}_{calc}$ (vedi Fig.\ref{fig:DTA}).
\begin{equation}
DTA = \min_{\vec{r}_{calc}} \left|\vec{r}_{meas} - \vec{r}_{calc}\right|
\end{equation}
L'utilizzo della distanza di accordo come criterio di confronto è consigliata dal protocollo ESTRO per le valutazioni di accettabilità delle zone ad alto gradiente ($\delta D > 3\%\,$mm$^{-1}$) (es. build-up delle PDD e penombre dei profili) .
\end{description}
...
\end{itemize}
\item Apertura dei collimatori comparabili con le dimensioni proiettate della sorgente.
\begin{itemize}
...
\end{itemize}
Queste scelte ricalcano le raccomandazioni dei protocolli di dosimetria per 3D-CRT per quanto riguarda l'utilizzo della camera a ionizzazione per i campi `grandi' (al di sopra di $4x4\,$cm$^2$) mentre considerano le nuove evidenze sperimentali \cite{Das2008a,Low2011} riguardo l'utilizzo di detector di piccole dimensioni per i campi piccoli ($\leq 5x5\,$cm$^2$). 
