...
\end{figure}
In Fig.\ref{fig:MLC_Regions} sono indicate le zone critiche in cui la particolare struttura del leaf (tip e tongue\&{}groove) influenza la distribuzione di dose specie in caso di sovrapposizione di diversi campi. \\
...
\caption{La somma di campi adiacenti sul bordo laterale del leaf evidenzia dei sottodaggi nelle zone di giunzione dovuti alla struttura tongue\&{}groove. I profili di dose sono acquisiti ad una profondità di $10\,$cm in fantoccio ad acqua con il detector PFD (IBA Dosimetry, vedi Tab.\ref{tab:OF_inter}).}
\label{fig:MLC_TG_effect}
\end{figure}

La scelta per quanto riguarda le misure effettuate per modellizzare l'MLC è dunque quella di utilizzare delle scansioni che esaltino l'effetto di leaf tip e di tongue\&{}groove come quelle mostrate nelle Fig.\ref{fig:MLC_tip_effect} e \ref{fig:MLC_TG_effect}. Per poter evidenziare in maniera affidabile questi effetti di composizione dei campi è necessario utilizzare dei detector che modifichino il meno possibile le penombre dei profili (i.e. con un volume sensibile minimo). In particolare sono stati utilizzati i diodi PFD e Razor (IBA Dosimetry) per le seguenti scansioni:

\begin{description}
\item[Effetto di \textit{leaf-tip}:]
\begin{itemize}
\phantom{item[-]}
\item[-] Profili lungo la direzione di movimento dei leaf (crossline) di campi quadrati collimati dai soli leaf con diodo PFD (IBA Dosimetry) in fantoccio ad acqua a profondità $10\,$cm. Queste misure permettono di calibrare finemente la posizione dei leaf all'interno del TPS.\footnote{Questa operazione è necessaria perché solitamente la posizione nominale dei leaf viene leggermente alterata sulla base di una calibrazione effettuata dalla ditta produttrice per far corrispondere il campo luce con il campo definito dai raggi.}.
...
\item[-] Profili di campi complementari come i precedenti a profondità $1.5\,$cm e $20\,$cm e traslati lateralmente lungo la direzione di movimento dei leaf in entrambe le direzioni per verificare la bontà della modellizzazione anche lontano dalle condizioni iniziali.
\end{itemize}

\item[Effetto di \textit{tongue\&{}groove:}]
\begin{itemize}
\phantom{item[-]}
\item[-] Profili di campi complementari adiacenti sul lato del bordo del leaf (Fig.\ref{fig:MLC_tip_effect}) con diodo PFD (IBA Dosimetry) lungo la direzione di perpendicolare rispetto al movimento dei leaf (inline) in fantoccio ad acqua a profondità $10\,$cm.
\item[-] Profili come i precedenti a profondità di $5\,$cm utilizzando entrambi i banchi di leaf.
\end{itemize}
\end{description}
