...
\subsection{Accuratezza richiesta per le tecniche avanzate (IMRT e VMAT)}
...
Seguendo le linee guida AAPM TG-120 citate nella sezione precedente \cite{Low2011} e tenendo conto delle raccomandazioni della ditta produttrice del TPS (Tab.\ref{tab:meas}), le scelte effettuate per le misure di PDD e profili di dose sono le seguenti:
\begin{itemize}
...
\end{itemize}
Queste scelte ricalcano le raccomandazioni dei consolidati protocolli di dosimetria per quanto riguarda l'utilizzo della camera a ionizzazione per i campi al di sopra di $4x4\,$cm$^2$ mentre considerano le nuove evidenze sperimentali \cite{Das2008a,Low2011} riguardo la necessità di utilizzo di detector di piccole dimensioni per i campi al di sotto di $5x5\,$cm$^2$.

Utilizzare un detector di piccole dimensioni nei profili evita l'effetto di smussamento delle penombre che si evidenzia impiegando le camere a ionizzazione (Fig.\ref{fig:pen_smear}).
\begin{figure}[!t]
\centering
\includegraphics[width=.6\textwidth]{./cap2/pen_smear.png}
\caption{Effetto di smussamento della penombra dei profili dovuto al volume esteso delle camere a ionizzazione confrontato con scansioni effettuate con detector di dimensioni ridotte.}
\label{fig:pen_smear}
\end{figure}
Questo è di cruciale importanza in quanto vedremo che la modellizzazione delle dimensioni della sorgente primaria è tanto più accurata quanto più lo sono le misure di penombra dei campi di piccole dimensioni.


Qui parla delle misure effettuate, con quali detector, perché, riporta la problematica degli OF e della risoluzione dei profili (mitigata dall'algoritmo di convoluzione). Mentre per gli OF parla del fatto che abbiamo richiesto l'utilizzo di altri detector, grafico, confronto con letteratura (Das) e scelta di non correggere il PFD (protocollo ancora non ufficiale) tuttavia segnala vagamente gli studi che ne parlano...
