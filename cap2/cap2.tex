...
\item[Distanza di accordo:] fissato un punto nello spazio individuato dal vettore $\vec{r}_{meas}$ corrispondente ad una certa dose misurata, la distanza di accordo rappresenta la minima distanza tra $\vec{r}_{meas}$ e tutti i punti aventi la medesima dose calcolata individuati dai vettori $\vec{r}_{calc}$.
\begin{equation}
DTA = \min_{\vec{r}_{calc}} \left|\vec{r}_{meas} - \vec{r}_{calc}\right|
\end{equation}
L'utilizzo della distanza di accordo come criterio di confronto è consigliata dal protocollo ESTRO per le valutazioni di accettabilità delle zone ad alto gradiente ($\delta D > 3\%\,$mm$^{-1}$) (es. build-up delle PDD e penombre dei profili) (vedi Fig.\ref{fig:DTA}).
\end{description}

\begin{figure}
\centering
\includegraphics[width=.6\textwidth]{./cap2/DTA.png}
